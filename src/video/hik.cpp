...
namespace video {
    ...
    bool HikCamera::initializeCamera(const std::string& target_sn) {
        ...
        while (!stop_signal_) {
            ...
            WUST_INFO(hik_logger_)
                << "Selecting camera at index " << sel << " (SN=" << target_sn << ")";
    ...
    void HikCamera::start() {
        ...
        if (trigger_type_ != TriggerType::Software) {
            capture_thread_ = wust_vl::common::concurrency::MonitoredThread::create(
                "HikCaptureThread",
            ...
            wust_vl::common::concurrency::ThreadManager::instance().registerThread(capture_thread_);
            process_thread_ = wust_vl::common::concurrency::MonitoredThread::create(
                "HikProcessThread",
                [this](wust_vl::common::concurrency::MonitoredThread::Ptr self) {
                    this->hikProcessLoop(self);
                }
            );
            wust_vl::common::concurrency::ThreadManager::instance().registerThread(process_thread_);
        }
    }
    ...
    void HikCamera::stop() {
        ...
        if (capture_thread_) {
            ...
            wust_vl::common::concurrency::ThreadManager::instance().unregisterThread(
                capture_thread_->getName()
            );
        }
        if (process_thread_) {
            process_thread_->stop();
            wust_vl::common::concurrency::ThreadManager::instance().unregisterThread(
                process_thread_->getName()
            );
        }
        if (camera_handle_) {
            MV_CC_StopGrabbing(camera_handle_);
