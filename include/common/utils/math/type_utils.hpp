...
/**
     ...
     */
template<typename T>
concept vector_arithmetic = std::is_arithmetic_v<base_type<
    T>> &&(std::is_same_v<base_type<T>, int> || std::is_same_v<base_type<T>, float> || std::is_same_v<base_type<T>, double>);
...
/**
     ...
     */
template<typename T>
concept vector_2_type = vector_2_type_sub_1<T> || vector_2_type_sub_2<T> || vector_2_type_sub_3<
    T> || vector_2_type_sub_4<T>;
...
/**
     ...
     */
template<typename T>
concept vector_3_type = vector_3_type_sub_1<T> || vector_3_type_sub_2<T> || vector_3_type_sub_3<
    T> || vector_3_type_sub_4<T>;
...
/**
     ...
     */
template<typename T>
requires(vector_2_type<T> || vector_3_type<T>) constexpr auto get_x(const T& v) noexcept {
...
/**
     ...
     */
template<typename T>
requires(vector_2_type<T> || vector_3_type<T>) constexpr auto get_y(const T& v) noexcept {
    if constexpr (vector_2_type_sub_1<T> || vector_3_type_sub_1<T>)
        return v.y;
    else if constexpr (vector_2_type_sub_2<T>)
        return v(0, 1);
...
constexpr auto get_z(const T& v) noexcept {
    if constexpr (vector_3_type_sub_1<T>)
        ...
        static_assert(details::always_false<T>, "Unsupported type for get_z (must be 3D vector)");
}
...
requires vector_3_type<
    Source> &&(std::same_as<std::remove_cvref_t<Target>, cv::Vec<typename Target::value_type, 3>> || std::same_as<std::remove_cvref_t<Target>, cv::Matx<typename Target::value_type, 3, 1>>)constexpr Target
    convert3d(const Source& v) noexcept {
