        ...
        namespace stringanything {
            ...
            public:
                ...
                void set_value(const std::string& name, T value, bool overwrite = true) {
                    ...
                    if (it != map_.end()) {
                        ...
                        auto hold =
                            std::any_cast<std::shared_ptr<StaticHolder<Decayed>>>(it->second.storage
                            );
                ...
                [[nodiscard]] std::optional<std::decay_t<T>> try_get_value(const std::string& name
                    ...
                    try {
                        auto holder =
                            std::any_cast<std::shared_ptr<StaticHolder<Decayed>>>(e.storage);
                ...
                template<typename T>
                [[nodiscard]] std::shared_ptr<std::decay_t<T>> get_ptr(const std::string& name
                ) const {
                    using Decayed = std::decay_t<T>;
                    const std::type_index ty = typeid(Decayed);
            ...
            private:
                struct Entry {
                    ...
                    Entry(Kind k, std::type_index t, std::any s):
                        kind(k),
                        type(t),
                        storage(std::move(s)) {}
                };
