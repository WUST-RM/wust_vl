...
namespace common {
    namespace concurrency {
        ...
        public:
            using Ptr = std::shared_ptr<MonitoredThread>;
            enum class Status { Running, Stopped, Hung, Paused };

            using Task = std::function<void(Ptr)>;

            static Ptr create(const std::string& name, Task task) {
                auto mt = Ptr(new MonitoredThread(name));
                mt->start(task);
                return mt;
            }
        ...
        public:
            ...
            void registerThread(const MonitoredThread::Ptr& t) {
                std::lock_guard<std::mutex> lk(mtx_);
